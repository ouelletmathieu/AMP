...
def get_random_conn(n_out, nedge, npoint):
    ...
    while not found_connected_to:
        ...
        connected_to = [random.sample(node_list, len(result[i])) for i in range(len(result)) ]
...
def compare_connection(connection1, connection2):
    ...
    dict_conn1, dict_conn2 = {p:set() for p in list_inside_node1}, {p:set() for p in list_inside_node2}
    ...
    for perm in itertools.permutations([i for i in range(len(list_inside_node1))]):
        ...
        if found:
            translation = {node1:list_inside_node2[perm[i]] for i,node1 in enumerate(list_inside_node1)}
...
def compare_distance(inside_node_1, inside_node_2, perm):
    ...
    in_healthy_x_2, in_healthy_y_2 = [inside_node_2[0][i] for i in perm], [inside_node_2[1][i] for i in perm]
    in_prion_x_2, in_prion_y_2 = [inside_node_2[2][i] for i in perm], [inside_node_2[3][i] for i in perm]

    dist_list = [math.sqrt( (x1-x2)**2 + (y1-y2)**2 )  for x1,y1,x2,y2 in zip(in_healthy_x_1,in_healthy_y_1,in_healthy_x_2,in_healthy_y_2)]
    dist_list.extend([math.sqrt( (x1-x2)**2 + (y1-y2)**2 )  for x1,y1,x2,y2 in zip(in_prion_x_1,in_prion_y_1,in_prion_x_2,in_prion_y_2)])
        ...
        if not db.is_locked(int(key_healthy)):
            ...
            for i_p, key_prion in enumerate(list_prion_key):
                ...
                if 'sols' in db.db_pairs[key_healthy][key_prion]:
                    for key,items in  db.db_pairs[key_healthy][key_prion]['sols'].items():
                        already_found_sol.append( (items["inside_node"], items["conn"] ) )
                ...
                if nb_already_found < nb_solution_wanted:
                    ...
                    Xs0 = [[p[0] for p in pos_healthy_test_trans],[p[1] for p in pos_healthy_test_trans]]
                    XsF = [[p[0] for p in pos_prion_no_loop],[p[1] for p in pos_prion_no_loop]]
                    ...
                    while nb_try < max_nb_try and nb_found<nb_solution_wanted:
                        ...
                        conn_unformated = get_random_conn(n_poly, nedge_in, nb_node_in)
                        conn = []
                        ...
                        okay = True
                        x0 = [[random.random() for i in range(nb_node_in)], [random.random() for i in range(nb_node_in)]]
                        #transform in matlab
                        ...
                        Xu_m, fu_m = eng.construct_network_c(Xs0_m, XsF_m, x0_m, conn_m, 1, nargout=2)
                ...
                if len(new_sol_list)>0:#add new solution if we found some
                    db.add_solution(key_healthy, key_prion, new_sol_list)
